<div align="center">

# 📑 多来源的每日壁纸滚动器 每日壁纸并设置的小工具
***DailyWallpaperRoller***
***[项目主页](https://www.yan-me.top/20260211/)***
</div>

<p align="center">
  <a href="https://github.com/TokenYan233/DailyWallpaperRoller/releases"><img src="https://img.shields.io/github/v/release/TokenYan233/DailyWallpaperRoller?color=orange&logo=github" alt="Release"></a>
  <a href="https://opensource.org/licenses/MIT"><img src="https://img.shields.io/badge/License-MIT-blue.svg" alt="License"></a>
  <a><img src="https://img.shields.io/badge/Language-aardio-red.svg" alt="Language"></a>
  <a><img src="https://img.shields.io/badge/Platform-Windows-0078d7.svg?logo=windows" alt="Platform"></a>
</p>

---
适配了cnbing，词霸，ONE一个、bing全球，三类的每日壁纸提供方，依次查询配置文件中提供方地址。
查询下载每日壁纸，并设置为本机壁纸。如果今天壁纸滚动完了，就往前推一天，最大-7天

<img width="667" height="554" alt="DailyWallpaperRoller" src="https://github.com/user-attachments/assets/081763c2-81bd-4396-b0b1-3d9c1e991b11" />

支持同时查询cnbing和bing全球（配置范例里默认是fr-FR，可以加地址也可以改）的其他国家的壁纸，

支持静默开关（显示界面勾选框），打开后支持静默执行。悄悄的查询下载更换，很方便很安静

有技术的可以自己加入计划任务几个小时跑一趟，不会的就把开机自启选上就行了
